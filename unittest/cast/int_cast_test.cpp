...
const lest::test module[] =
{
    ...
    CASE("int_cast() accepts any integer type (except bool)")
    {
        ...
        EXPECT((0 == int_cast<signed char>(0)));
        EXPECT((0 == int_cast<unsigned char>(0)));
        EXPECT((0 == int_cast<short>(0)));
        EXPECT((0 == int_cast<unsigned short>(0)));
        EXPECT((0 == int_cast<int>(0)));
        EXPECT((0 == int_cast<unsigned int>(0)));
        EXPECT((0 == int_cast<long>(0)));
        EXPECT((0 == int_cast<unsigned long>(0)));
        EXPECT((0 == int_cast<long long>(0)));
        EXPECT((0 == int_cast<unsigned long long>(0)));
        EXPECT((0 == int_cast<char>(0)));
        EXPECT((0 == int_cast<wchar_t>(0)));
        EXPECT((0 == int_cast<char16_t>(0)));
        EXPECT((0 == int_cast<char32_t>(0)));
    },
    ...
    CASE("int_cast() can cast maximum value of FROM type")
    {
        ...
        long long imax;
        {
            ...
            EXPECT((imax == int_cast<long long>(std::numeric_limits<type>::max())));
        }
        {
            using type = short;
            imax = std::numeric_limits<type>::max();
        ...
        unsigned long long umax;
        {
            ...
            EXPECT((umax == int_cast<unsigned long long>(std::numeric_limits<type>::max())));
        }
        {
    ...
    CASE("int_cast() can cast minimum value of FROM type")
    {
        ...
        long long imin;
        {
            ...
            EXPECT((imin == int_cast<long long>(std::numeric_limits<type>::lowest())));
        }
        {
            using type = short;
            imin = std::numeric_limits<type>::lowest();
        ...
        unsigned long long umin;
        {
            ...
            EXPECT((umin == int_cast<unsigned long long>(std::numeric_limits<type>::lowest())));
        }
        {
            using type = unsigned short;
            umin = std::numeric_limits<type>::lowest();
    ...
    CASE("several use cases int_cast() successes to cast")
    {
...

        {
            signed char from = 12;
            int to = 12;
            EXPECT((to == int_cast<decltype(to)>(from)));
        }
        {
            int from = 12;
            signed char to = 12;
    ...
    CASE("several use cases int_cast() throws std::out_of_range")
    {
...

        {
            int from = 300;
            signed char to;
            EXPECT_THROWS_AS((int_cast<decltype(to)>(from)), std::out_of_range);
        }
        {
            int from = 1800000000;
            short to;
